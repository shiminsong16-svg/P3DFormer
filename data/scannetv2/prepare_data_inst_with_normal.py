...
def f_test(fn):
    ...
    print('Saved processed scan and normals for ' + fn)
...
def f(fn):
    ...
    torch.save(superpoint, fn[:-15] + '_superpoints.pth')
    normals = vertex_normal(vertices.numpy()[:, :3], faces.numpy())
    torch.save(
        (coords, colors, superpoint.numpy(), sem_labels, instance_labels),
        fn[:-15] + '_inst_nostuff.pth',
    )
    torch.save(normals, fn[:-15] + '_normals.pth')
    print('Saved processed scan, superpoints, and normals for ' + fn)
